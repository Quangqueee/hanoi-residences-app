...
type Props = {
  /** `tab` sits in the bottom tab; `stack` is the profile stack with Đóng. */
  variant: 'tab' | 'stack';
};
...
export function LandlordApartmentsPanel({ variant }: Props) {
  ...
  const onRequestPush = (apt: Apartment) => {
    ...
    Alert.alert('Xin đẩy tin', 'Tin sẽ được đẩy lên đầu danh sách. Tiếp tục?', [
      { text: 'Hủy', style: 'cancel' },
      {
        text: 'Xin đẩy',
        onPress: () => {
  ...
  const fabBottom = isTab
    ? TAB_BAR_BODY_HEIGHT + insets.bottom + 16
    : Math.max(insets.bottom, 16) + 8;
  const listBottom = fabBottom + 56;

  return (
    <View
      className="flex-1 bg-white"
      style={{ paddingTop: isTab ? insets.top : 0 }}>
      {isTab ? (
        <View className="px-6 pb-3 pt-3">
          ...
          <Text className="mt-1 text-[14px] leading-5 text-hoteliq-gray">
            Đăng tin, đổi trạng thái phòng và xin đẩy listing.
          </Text>
        </View>
      ) : (
              ...
              <View className="flex-row gap-3 p-3">
                {thumb ? (
                  ...
                  <View className="h-20 w-20 items-center justify-center rounded-[10px] bg-hoteliq-chip">
                    <Text className="text-[11px] text-hoteliq-gray">
                      No img
                    </Text>
                  </View>
                )}
      ...
      <Pressable
        ...
        className="absolute left-6 right-6 h-12 items-center justify-center rounded-full bg-hoteliq-ink"
        style={({ pressed }) => ({
